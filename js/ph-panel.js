/**
 * @name  phPanel
 * @description Phoast Panel Directive.  Implements <ph-panel> which
 *  displays a rounded panel with various effects.
 * @author Carlos Justiniano
 */
angular.module('presentationApp')
  .directive('phPanel', function() {
    'use strict';
    var defaults = {
      ...
      panelTitleFontSize: '1em', /* default panel title font size */
    ...
    return {
      template: '<div class="ph-panel">' +
        '<div class="ph-panel-title">{{ title }}</div>' +
        '<div class="ph-panel-content"><div ng-transclude></div></div>' +
        '</div>',
      restrict: 'AE',
      transclude: true,
      scope: {},
      link: function postLink(scope, element, attrs) {
        ...
        scope.minititle = attrs.minititle;

        var panel = element.find('.ph-panel');
        panel.css({
        ...
        if (scope.title !== undefined && scope.title !== '') {
          var panelTitle = element.find('.ph-panel-title');
          panelTitle.css({
            //'cursor': 'pointer',
        ...
        var panelContent = element.find('.ph-panel-content');
        panelContent.css({
          'margin': '5px',
          'padding': '8px',
          'color': 'black'
